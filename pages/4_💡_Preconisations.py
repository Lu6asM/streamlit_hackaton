...
ref_tm, rec_tm, delta_tm = calcul_tendance("TM")
ref_gel, rec_gel, delta_gel = calcul_tendance("NBJGELEE")
ref_can, rec_can, delta_can = calcul_tendance("NBJTX30")
ref_rr, rec_rr, delta_rr = calcul_tendance("RR")
...
with st.expander("☀️ **Sécheresse & Évapotranspiration** — Stress hydrique croissant"):
    ref_etp, rec_etp, delta_etp = calcul_tendance("ETP")
    st.markdown(f"""
...
with st.expander("⛈️ **Événements extrêmes** — Orages et phénomènes violents"):
    ref_orag, rec_orag, delta_orag = calcul_tendance("NBJORAG")
    st.markdown(f"""
